...
- **Fix-forward (P0.5 follow-up, 2026-05-09)**:
  ...
  - **U1-VR blocker discovered (NOT fixed in P0.5; carried forward as U1-VR-side TBD)**: agent-browser's `--cdp <port>` mode does NOT reliably maintain tab/state across multiple invocations against an externally-launched Electron process. After `agent-browser --cdp 9222 tab t2` reports a successful switch, the next `agent-browser --cdp 9222 screenshot <path>` command silently produces no output and writes no file. The daemon-model (`agent-browser connect 9222 …`) launches its own Chromium with `about:blank` rather than attaching to the running Electron's tabs. Snapshot reads via `--cdp 9222` DO work and confirm the OhMyToken dashboard renders correctly under the seeded profile — so the wiring (Electron + CDP + dist build + HOME_OVERRIDE + stabilization vars) is functional. The blocker is purely on the agent-browser side, and per §C5 + `.claude/rules/agent-browser-qa.md` agent-browser is the only allowed automation tool (Playwright / chrome-remote-interface / puppeteer are forbidden, no exceptions). U1-VR resolution path: (a) re-validate workflow patterns from the latest `agent-browser skills get electron` (especially around session/tab persistence), (b) report the screenshot-after-tab-switch silent-fail upstream as an agent-browser issue and align on the supported pattern, (c) if the daemon-model exposes a different attach flow than `connect <port>` for external Electron, use that pattern. Versions tested during P0.5: 0.26.0 (initial install) and 0.27.0 (upgraded 2026-05-09); both exhibit the same silent-fail. This is captured as the FIRST item in U1-VR Step 0.
