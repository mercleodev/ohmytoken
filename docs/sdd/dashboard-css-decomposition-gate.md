...
- **Fix-forward (P0.5 follow-up, 2026-05-09)**:
  ...
  - **U1-VR blocker discovered (NOT fixed in P0.5; carried forward as U1-VR-side TBD)**: `agent-browser 0.26.0`'s `--cdp <port>` mode does NOT reliably maintain tab/state across multiple invocations against an externally-launched Electron process. After `agent-browser --cdp 9222 tab t2` reports a successful switch, the next `agent-browser --cdp 9222 screenshot <path>` command silently produces no output and writes no file. The daemon-model (`agent-browser connect 9222 …`) launches its own Chromium with `about:blank` rather than attaching to the running Electron's tabs. Snapshot reads via `--cdp 9222` DO work and confirm the OhMyToken dashboard renders correctly under the seeded profile — so the wiring (Electron + CDP + dist build + HOME_OVERRIDE + stabilization vars) is functional. The blocker is purely on the agent-browser side. U1-VR will need to either (a) upgrade agent-browser, (b) replace agent-browser screenshot with raw CDP via Node `chrome-remote-interface` or Playwright, or (c) drive captures manually with the user at the keyboard. All three are valid resolutions; (b) is preferred because it keeps U1-VR reproducible. This is captured as the FIRST item in U1-VR Step 0.
